## THIS DOCUMENT WAS EDITED USING AI TO DEBUG A SEGFAULT ISSUE. DO NOT EDIT MANUALLY.
...
class VisionSearchServer(Node):
    ...
    def recognize_face(self, person_crop):
        print("recognize_face() called", flush=True)
        return None
    ...
    def execute_callback(self, goal_handle):
        ...
        self.get_logger().info(f'New search goal accepted for target: {self.current_target}. Handing execution over to main thread loop...')
        
        self.goal_handle = goal_handle
        self.target_found = False
        
        # update to target
        self.db_path = f"/workspaces/ros_ws/src/project/config/face_id/{self.current_target.lower()}"
        ...
        while rclpy.ok():
            ...
            if node.search_active:
                ...
                if node.accumulated_yaw >= (math.pi):
                    node.get_logger().info(f"Completed exact 180-degree rotation. Target '{node.current_target}' not found.")
                ...
                if frame is not None:
                    ...
                    if (current_time - last_inference_time) >= inference_interval:
                        ...
                        # results = node.yolo(frame, conf=0.6)
                        
                        # person_centered = False
                        # target_box = None
                        print("========== START OF INFERENCE ==========", flush=True)

                        print("1. About to run YOLO", flush=True)
                        results = node.yolo(frame, conf=0.6)
                        print("2. YOLO returned", flush=True)

                        person_centered = False
                        target_box = None

                        print("3. About to iterate results", flush=True)

                        for r in results:
                            print("4. Entered results loop", flush=True)

                            for box in r.boxes:
                                print("5. Entered boxes loop", flush=True)

                                if int(box.cls[0]) == 0:
                                    print("6. Found person", flush=True)

                                    x1, y1, x2, y2 = map(int, box.xyxy[0])
                                    box_center_x = (x1 + x2) // 2

                                    if abs(box_center_x - frame_center_x) <= allowed_offset:
                                        print("7. Person centered", flush=True)

                                        person_centered = True
                                        target_box = (x1, y1, x2, y2)
                                        break

                        print("8. Finished processing results", flush=True)
                        ...
                        if current_time < ignore_center_until:
                            ...
                            if person_centered and target_box:
                                ...
                                if time_verifying > 15.0:
                                    node.get_logger().info("15-second timeout reached. Target unrecognizable. Resuming sweep.")
                                    is_verifying = False
                                    
                                    ignore_center_until = current_time + 2.5 
                                    node.cmd_pub.publish(rotate_msg)
                                else:
                                    ...
                                    with node.lock:
                                        fresh_frame = node.latest_frame.copy() if node.latest_frame is not None else None
                ...
                if node.target_found:
                    node.get_logger().info(f"Target '{node.current_target}' found!")
